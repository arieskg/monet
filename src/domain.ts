import type { Candidate, Source, SourceMapping, Status, TaxonomyEntry, Workspace } from "../shared/model.js";

export type * from "../shared/model.js";
export { resolveThemeTokenSet, resolveTokenSet } from "../shared/tokens.js";
...
export function searchWorkspace(workspace: Workspace, query: string): SearchResult[] {
  ...
  return [
    ...
    ...workspace.themes.filter((item) => contains(item.name, item.id, Object.keys(item.overrides))).map((item) => ({ type: "Theme", id: item.id, title: item.name, description: `${Object.keys(item.overrides).length} Foundation overrides`, route: "/themes" })),
