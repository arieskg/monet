import { componentDecisionLabels, componentDecisionStatus, markdownExcerpt, primitiveEntries, resolveThemeTokenSet, taxonomyEntries, type MarkdownDocument, type Principle, type Workspace } from "./domain";
...
function workspaceMarkdown(workspace: Workspace): string {
  ...
  return [
    "# Monet Design System", "", `Resolved theme: ${workspace.themes.find((theme) => theme.id === workspace.activeThemeId)?.name ?? workspace.activeThemeId}. Theme values are Base Monet plus override-only theme data.`, "", "## Principles", "",
    ...
    "## Themes", "", ...workspace.themes.map((item) => `- **${item.name}**${item.id === workspace.defaultThemeId ? " (default)" : ""} — ${Object.keys(item.overrides).length} overrides`), "",
...
export function buildExportCollections(workspace: Workspace): ExportCollection[] {
  ...
  const themeData = workspace.themes.map((theme) => ({ id: theme.id, name: theme.name, overrides: theme.overrides }));
  return [
    ...
    { id: "tokens", label: "Tokens", description: "Resolved canonical values and any reference issues.", count: workspace.resolvedTokens.length, data: { tokens: workspace.resolvedTokens, issues: workspace.tokenIssues }, markdown: ["# Monet Tokens", "", ...workspace.resolvedTokens.map((item) => `- \`${item.name}\` = \`${String(item.resolved_value ?? item.value)}\`${item.description ? ` — ${item.description}` : ""}`), ""].join("\n") },
    ...
    { id: "themes", label: "Themes", description: "Product-specific Foundation overrides only; base values are never copied.", count: workspace.themes.length, data: themeData, markdown: ["# Monet Themes", "", "Theme files contain overrides only. Resolve them as Base Monet + theme overrides.", "", ...workspace.themes.flatMap((theme) => [`## ${theme.name}${theme.id === workspace.defaultThemeId ? " (default)" : ""}`, "", ...Object.entries(theme.overrides).map(([name, value]) => `- \`${name}\` = \`${String(value)}\``), ...(Object.keys(theme.overrides).length ? [] : ["_No overrides._"]), ""])].join("\n") },
...
export function workspaceWithTheme(workspace: Workspace, themeId: string): Workspace {
  const theme = workspace.themes.find((item) => item.id === themeId) ?? workspace.themes.find((item) => item.id === workspace.defaultThemeId) ?? null;
  const resolution = resolveThemeTokenSet(workspace.baseResolvedTokens, theme);
  return { ...workspace, activeThemeId: theme?.id ?? workspace.defaultThemeId, resolvedTokens: resolution.tokens, tokenIssues: resolution.issues };
}
