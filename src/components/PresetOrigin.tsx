...
export function PresetOrigin() {
  ...
  useEffect(() => { let live = true; void api.presetOrigin().then((r) => { if (live) setReceipt(r); }).catch(() => { if (live) setError("Preset provenance could not be read."); }); return () => { live = false; }; }, []);
