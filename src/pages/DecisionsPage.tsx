...
export function AppliedProposals({ receipts }: { receipts: ApplicationReceipt[] }) {
  ...
  return <section className="decision-applications" aria-label="Applied Gap proposals">
    ...
    <ul>{receipts.map((receipt) => <li key={receipt.id}>
      ...
      <span className={`status-pill ${receipt.outcome === "applied" ? "proposal-applied" : "proposal-rejected"}`}>{receipt.outcome === "applied" ? "Applied" : receipt.recovered ? "Rolled back at startup" : "Rolled back"}</span>
...
export function DecisionsPage() {
  ...
  useEffect(() => { let active = true; api.applications().then((value) => { if (active) setReceipts(value); }).catch(() => undefined); return () => { active = false; }; }, []);
