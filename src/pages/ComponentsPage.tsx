...
export function ComponentsPage() {
  ...
  return <div className="page components-page"><PageHeader eyebrow="Lightweight decisions" title="Components" description="Choose inspiration, record a few component-specific preferences, and inherit the rest from Monet." />
    <div className="component-workspace"><TaxonomyBrowser current={id} /><section className="component-detail">{!entry || !draft ? entries.length === 0 ? <div className="component-index"><span className="eyebrow">Empty taxonomy</span><h2>This workspace names no components yet.</h2><p>The taxonomy is the workspace's own list of concepts, in <code>taxonomy/components.json</code>. External libraries map into it; they never define it. Copy the bundled starter workspace to begin from a complete one, or write the file yourself — <code>docs/WORKSPACE.md</code> describes the shape.</p><Link className="button" to="/settings">Workspace settings</Link></div> : <div className="component-index"><span className="eyebrow">{entries.length} canonical concepts</span><h2>Choose a component to understand and decide.</h2><p>The taxonomy belongs to Monet. External libraries map into it without defining its structure.</p><div className="category-summary">{workspace?.taxonomy.map((category) => <div key={category.id}><b>{category.name}</b><span>{category.entries.length}</span><p>{category.entries.map((item) => item.name).join(" · ")}</p></div>)}</div></div> : <>
