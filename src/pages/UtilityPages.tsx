import { workspaceIsEmpty } from "../domain";
...
function WorkspaceSection() {
  ...
  return <section className="settings-section">
    ...
    <dl className="settings-facts">
      ...
      <div><dt>Origin</dt><dd>{environment ? environment.bundled ? "The bundled example that ships with Monet. It is also the fixture Monet's own tests assert against, so editing it in place can break the test suite and can conflict when you pull updates." : "A workspace you own." : "The file service did not report an origin."}</dd></div>
    </dl>
    {(environment?.bundled || (workspace && workspaceIsEmpty(workspace))) && <>
      <p className="settings-note">Stop the running dev server with Ctrl+C, then run these commands from the Monet repository root. Use a destination that does not already exist.</p>
...
function AppearanceSection() {
  ...
  return <section className="settings-section">
    ...
    <div className="settings-choice" role="group" aria-label="Appearance">
      {APPEARANCES.map((item) => <button key={item} type="button" aria-pressed={appearance === item} className={appearance === item ? "active" : ""} onClick={() => setAppearance(item)}>
...
function OptionalFeaturesSection() {
  ...
  return <section className="settings-section">
    ...
    <p className="settings-note">Monet does not bundle, depend on, or prefer a provider. It invokes the command with a prompt on stdin and reads the JSON it writes; the built-in argument shape matches the Codex CLI, and another provider works behind a small wrapper script. docs/ARCHITECTURE.md documents the exact call.</p>
  </section>;
}
