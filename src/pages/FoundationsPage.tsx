...
export function FoundationsPage() {
  ...
  return <div className="page"><PageHeader eyebrow="Visual language" title="Foundations" description="Design intent and the machine-readable token values that implement it." />
    <div className="workspace-layout"><aside className="collection-rail">{workspace?.foundations.map((item) => <Link key={item.id} className={item.id === id ? "active" : ""} to={`/foundations/${item.id}`}><span><b>{item.name}</b><small>{item.tokens.length} tokens</small></span><StatusPill value={item.status} /></Link>)}</aside>
      <section className="editor-panel">{!draft ? (workspace?.foundations.length ?? 0) === 0
        ? <div className="blank-editor"><span className="eyebrow">No foundations yet</span><h2>Colour, spacing, type — the values everything else refers to.</h2><p>Foundations hold the named values a design system is built from, and the reasoning behind them. Monet reads them from one JSON file per foundation in your workspace; the quickest start is to copy the bundled example and edit its files. <code>docs/WORKSPACE.md</code> describes the shape if you would rather write your own.</p><Link className="button" to="/settings">Workspace settings</Link></div>
        : <div className="blank-editor"><span className="eyebrow">Choose a foundation</span><h2>Intent first, values when useful.</h2><p>Tokens add precision without forcing every foundation into a token system.</p></div> : <>
