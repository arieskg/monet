...
import { resolveTokenSet, slugify, type Foundation, type Token, type TokenLevel, type TokenType } from "../domain";
...
function parseTokenImport(value: unknown, foundation: Foundation): Token[] {
  ...
  return records.map((record, index) => {
    ...
    names.add(name);
    ids.add(id);
    return { id, name, foundation: foundation.id, type, level, value: tokenValue, description: typeof candidate.description === "string" ? candidate.description : "", alias: typeof candidate.alias === "string" && candidate.alias ? candidate.alias : undefined, order: foundation.tokens.length + index };
  });
}
