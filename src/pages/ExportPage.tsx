...
export function ExportPage() {
  ...
  const resolvedWorkspace = workspaceWithTheme(workspace, selectedThemeId);
  ...
  const exportWholeWorkspace = (format: ExportFormat) => download(`monet-design-system-${resolvedWorkspace.activeThemeId}.${format}`, serializeWorkspace(resolvedWorkspace, format), mimeTypes[format]);
  return <div className="page export-page"><PageHeader eyebrow="Portable output" title="Export" description="Download the full workspace or one independent collection as Markdown, YAML, or JSON." />
    <section className="export-hero"><div><span className="eyebrow">Agent entrypoint</span><h2>Resolved intent, without a framework dependency.</h2><p>Choose a theme and export Base Monet plus its overrides. The base files remain unchanged, while resolved token values retain their origin metadata.</p><label className="export-theme-select">Resolve theme<select value={selectedThemeId} onChange={(event) => setThemeId(event.target.value)}>{workspace.themes.map((theme) => <option value={theme.id} key={theme.id}>{theme.name}{theme.id === workspace.defaultThemeId ? " · default" : ""}</option>)}</select></label><div className="button-row">{(["md", "yaml", "json"] as const).map((format) => <button className={format === "md" ? "button primary" : "button ghost"} key={format} onClick={() => exportWholeWorkspace(format)}>Full {formatLabels[format]}</button>)}</div></div><div className="export-stats"><div><strong>{workspace.principles.length}</strong><span>principles</span></div><div><strong>{resolvedWorkspace.resolvedTokens.length}</strong><span>resolved tokens</span></div><div><strong>{workspace.themes.length}</strong><span>themes</span></div><div><strong>{selected.length}</strong><span>component decisions</span></div></div></section>
...
├── principles/               title + Markdown body
	...
	├── themes/                   override-only product themes
	├── tokens/                   generated resolved token exports
	│   ├── tokens.json            default-theme resolved set
	│   └── themes/                one resolved set per theme, with origins
