...
function SurfaceEditor({ id, captureId }: { id?: string; captureId?: string }) {
  ...
  return <div className="page surfaces-page">
    ...
    {preview && <>
      ...
      <fieldset className="surface-panel" disabled={busy || historical}><legend>Approve mappings</legend><p>Equal values suggest candidates, not semantic roles. Selecting a token approves only this declaration. Custom-property mappings affect its existing consumers through CSS; their roles are not inferred.</p>{preview.capture?.strategy === "computed" && <p className="surface-notice">These values came from browser computed styles at one viewport and appearance, including inherited and default values. They do not establish authored declarations, token usage or semantic roles. Sanitization and mapping substitutions may change the observed appearance.</p>}
