...
function AnalysisPanel({ analysis, references, busy, onAnalyze, onSuggestion }: { analysis: ReferenceCollectionAnalysis; references: Reference[]; busy: boolean; onAnalyze: () => void; onSuggestion: (id: string, status: ReferenceSuggestionStatus) => void }) {
  ...
  return <section className="reference-analysis-panel" aria-labelledby="reference-analysis-heading">
    <header><div><span className="eyebrow">Collection intelligence</span><h2 id="reference-analysis-heading">Recurring preferences</h2><p>{analysis.summary || "Analyze the collection to find repeated visual preferences and design-system opportunities."}</p></div><button className="button primary" disabled={busy || !references.length} onClick={onAnalyze}>{busy ? "Analyzing…" : analysis.analyzed_at ? "Analyze again" : "Analyze References"}</button></header>
...
export function ReferencesPage() {
  ...
  return <div className="page references-page">
    <PageHeader eyebrow="Inspiration" title="References" description="Save specific visual examples, record what matters to you, and let Monet build a retrievable preference memory." action={<div className="button-row"><button className="button ghost" disabled={!references.length || analysisBusy} onClick={() => void analyzeCollection()}>{analysisBusy ? "Analyzing…" : "Analyze References"}</button><button className="button primary" onClick={() => setAdding(true)}>Add reference</button></div>} />
